...
class _SignUpOnboardingPageState extends State<SignUpOnboardingPage> {
  ...
  Future<void> _loadOptions() async {
    try {
      ...
      // you can show a snackbar, but don't block signup entirely
  ...
  Future<void> _addCustomSkill() async {
    ...
    setState(() {
      ...
      _selectedSkillKeys.remove('other'); // don't keep literal 'other'
      _selectedSkillKeys.add(key);
  ...
  Future<void> _handleSubmit() async {
    ...
    if (_selectedRoleKey == 'other') {
      _showSnack('Please add a custom role instead of leaving "Other" selected.');
      return;
    }
    // For skills, we ignore literal 'other' key when sending to backend
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      appBar: AppBar(
        title: const Text('Create your DevSta account'),
                    ...
                    children: [
                            ...
                            Expanded(
                              child: TextField(
                                ...
                                decoration: const InputDecoration(
                                  labelText: 'Custom role (e.g. SRE, Data Eng)',
                      ...
                      Wrap(
                        ...
                        children: _skillOptions.map((s) {
                          final selected = _selectedSkillKeys.contains(s.key);
                          return FilterChip(
                            label: Text(s.label),
                            selected: selected,
                            onSelected: (_) {
                              if (s.key == 'other') {
                                // toggle "Other"
                                setState(() {
                                  if (_selectedSkillKeys.contains('other')) {
                      ...
                      SizedBox(
                        ...
                        child: ElevatedButton(
                          onPressed: _loadingSubmit ? null : _handleSubmit,
                          style: ElevatedButton.styleFrom(
                            backgroundColor: primary,
